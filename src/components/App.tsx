...
export function App() {
    ...
    const importSession = (file: File) => {
        const hasWork = turns.length > 0 || drafting.stories.length > 0 || elicitation.candidates.length > 0;
        if (hasWork && !window.confirm("Importing will replace your current session. Export it first if you want to keep it. Continue?")) {
            return;
        }
        const reader = new FileReader();
        reader.onload = () => {
            try {
                const { session, regroundedCount } = parseImportedSession(String(reader.result));
                setStage(session.stage as StageKey);
                setTurns(session.turns);
                setElicitation(session.elicitation);
                setDrafting(session.drafting);
                setGoals(session.goals);
                setApproved(session.approved);
                setScopePrefix(session.scopePrefix ?? "US");
                if (regroundedCount > 0) {
                    window.alert(`Session imported. ${regroundedCount} item(s) had a quote that no longer matches the transcript and were marked ungrounded for your review.`);
                }
            } catch (err) {
        ...
        <div style={page}>
            ...
            <header style={header}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <button style={ui.btnPrimary} onClick={() => setShowGuide(true)}>
                        Guide
                    </button>
                    <label style={{ ...ui.btnSecondary, display: "inline-flex", alignItems: "center", justifyContent: "center", lineHeight: 1, height: 34, boxSizing: "border-box", cursor: "pointer" }}>
                        Import .json
                        <input
                            type="file"
                    ...
                    <button style={{ ...ui.btnSecondary, display: "inline-flex", alignItems: "center", justifyContent: "center", lineHeight: 1, height: 34, boxSizing: "border-box" }} onClick={() => downloadSessionJson(session)}>
                        Export .json
                    </button>
